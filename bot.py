...
def load_mosques_from_sheet(url):
    now = time.time()
    if mosques_cache["data"] is not None and now - mosques_cache["loaded_at"] < SHEET_CACHE_SECONDS:
        return mosques_cache["data"]

    csv_text = request_text(url)
...
def main_keyboard(mosques):
    return {
        "keyboard": [
            [{"text": BTN_ENTRY_TIMES}],
            [{"text": BTN_MOSQUE_TIMES}],
            [{"text": BTN_MOSQUE_LOCATIONS}],
            [{"text": BTN_NEAREST_MOSQUE}],
        ],
        "resize_keyboard": True,
    }


def mosque_keyboard(mosques):
    rows = []
    for index in range(0, len(mosques), 2):
        rows.append([{"text": mosque["name"]} for mosque in mosques[index : index + 2]])
    rows.append([{"text": BTN_BACK}, {"text": BTN_MAIN_MENU}])
    return {"keyboard": rows, "resize_keyboard": True, "one_time_keyboard": True}


def location_request_keyboard():
    return {
        "keyboard": [[{"text": BTN_SEND_LOCATION, "request_location": True}], [{"text": BTN_BACK}, {"text": BTN_MAIN_MENU}]],
        "resize_keyboard": True,
...
def format_times(title, mosque, target_date, times):
    lines = [
        f"🕋 <b>{title}</b>",
        f"Shahar: <b>{KOKAND_LABEL}</b>",
        f"Masjid: <b>{mosque['name']}</b>",
        f"Sana: <b>{target_date.strftime('%d.%m.%Y')}</b>",
        "",
    ]
    location = mosque.get("location") or {}
    if location.get("address"):
        lines.append(f"Manzil: <b>{location['address']}</b>")
    if location.get("lat") and location.get("lon"):
        lines.append(f"Xarita: https://maps.google.com/?q={location['lat']},{location['lon']}")
    lines.append("")
    lines.extend(f"{name}: <b>{times[name]}</b>" for name in PRAYER_NAMES if name in times)
    if mosque.get("note"):
        lines.extend(["", mosque["note"]])
    return "\n".join(lines)


def format_entry_times(target_date, times):
    lines = [
        "🕋 <b>Kirish vaqtlari</b>",
        f"Shahar: <b>{KOKAND_LABEL}</b>",
        f"Sana: <b>{target_date.strftime('%d.%m.%Y')}</b>",
...
def format_prayer_notification(prayer_name, prayer_time):
    return (
        f"🕋 <b>{prayer_name} vaqti kirdi</b>\n\n"
        f"Qo'qon shahri\n"
        f"Vaqt: <b>{prayer_time}</b>"
    )


def format_mosque_prayer_times(mosque):
    times = mosque.get("prayer_times") or {}
    lines = [
        "🕌 <b>Masjiddagi namoz vaqtlari</b>",
        f"Masjid: <b>{mosque['name']}</b>",
        "",
    ]
    if not times:
        lines.append("Bu masjid uchun vaqtlar hali kiritilmagan.")
        return "\n".join(lines)
    lines.extend(f"{name}: <b>{times[name]}</b>" for name in PRAYER_NAMES if name in times)
    return "\n".join(lines)


def format_mosque_location(mosque):
    location = mosque.get("location") or {}
    lines = ["📍 <b>Masjid joylashuvi</b>", f"Masjid: <b>{mosque['name']}</b>", ""]
    if location.get("address"):
        lines.append(f"Manzil: <b>{location['address']}</b>")
    if location.get("lat") and location.get("lon"):
        lines.append(f"Xarita: https://maps.google.com/?q={location['lat']},{location['lon']}")
    if len(lines) == 3:
        lines.append("Bu masjid lokatsiyasi hali kiritilmagan.")
    return "\n".join(lines)


def send_mosque_location(token, chat_id, mosque, mosques):
    send_message(token, chat_id, format_mosque_location(mosque), main_keyboard(mosques))
    location = mosque.get("location") or {}
    if location.get("lat") and location.get("lon"):
        send_location(token, chat_id, float(location["lat"]), float(location["lon"]), main_keyboard(mosques))
...
def find_mosque(mosques, text):
    ...
    for mosque in mosques:
        if normalized in {normalize_text(mosque["id"]), normalize_text(mosque["name"])}:
            return mosque
    return None
...
def handle_update(token, update, users, mosques):
    ...
    if "location" in message:
        ...
        if not mosque:
            send_message(token, chat_id, "Hali masjid lokatsiyalari kiritilmagan.", main_keyboard(mosques))
            return
        response = [
            "🧭 <b>Eng yaqin masjid</b>",
            f"Masjid: <b>{mosque['name']}</b>",
            f"Masofa: <b>{distance:.2f} km</b>",
            "",
            format_mosque_location(mosque),
        ]
        send_message(token, chat_id, "\n".join(response), main_keyboard(mosques))
        location_data = mosque.get("location") or {}
        if location_data.get("lat") and location_data.get("lon"):
            send_location(token, chat_id, float(location_data["lat"]), float(location_data["lon"]), main_keyboard(mosques))
        return

    if text.startswith(("/admin", "/mosqueids", "/setlocation", "/setmasjidtime", "/clearmasjidtime")):
        if handle_admin_command(token, chat_id, text, mosques):
            return

    if text in {"/start", "/help"}:
        send_message(token, chat_id, help_text(), main_keyboard(mosques))
        return

    if text in {BTN_MAIN_MENU, BTN_BACK, "Bosh menyu", "Ortga"}:
        users.setdefault(chat_id, {})["mode"] = "menu"
        save_users(users)
        send_message(token, chat_id, "🏠 <b>Bosh menyu</b>", main_keyboard(mosques))
        return

    if text in {BTN_ENTRY_TIMES, "Kirish vaqtlari"}:
        times = prayer_times_by_kokand(today)
        send_message(token, chat_id, format_entry_times(today, times), main_keyboard(mosques))
        return

    if text in {BTN_MOSQUE_TIMES, "Masjidlardagi vaqtlar"}:
        users.setdefault(chat_id, {})["mode"] = "mosque_times"
        save_users(users)
        send_message(token, chat_id, "🕌 <b>Masjidni tanlang</b>", mosque_keyboard(mosques))
        return

    if text in {BTN_MOSQUE_LOCATIONS, "Masjid Joylashuvlari"}:
        users.setdefault(chat_id, {})["mode"] = "mosque_locations"
        save_users(users)
        send_message(token, chat_id, "📍 <b>Joylashuvini ko'rish uchun masjidni tanlang</b>", mosque_keyboard(mosques))
        return

    if text in {BTN_NEAREST_MOSQUE, "Men turgan joyga eng yaqin masjid"}:
        users.setdefault(chat_id, {})["mode"] = "nearest"
        save_users(users)
        send_message(token, chat_id, "🧭 <b>Eng yaqin masjidni topish uchun lokatsiyangizni yuboring</b>", location_request_keyboard())
        return
    ...
    if mosque:
        ...
        if mode == "mosque_locations":
            send_mosque_location(token, chat_id, mosque, mosques)
            return
        send_message(token, chat_id, format_mosque_prayer_times(mosque), main_keyboard(mosques))
        return
    ...
    send_message(token, chat_id, "Tushunmadim. /help ni yuboring yoki menyudan tanlang.", main_keyboard(mosques))
...
def send_saved_mosque_times(token, chat_id, users, mosques, target_date, title):
    ...
    if not mosque:
        send_message(token, chat_id, "Avval masjid tanlang.", mosque_keyboard(mosques))
        return
    send_prayer_times(token, chat_id, mosque, target_date, title, mosques)


def send_prayer_times(token, chat_id, mosque, target_date, title, mosques):
    ...
    send_message(token, chat_id, format_times(title, mosque, target_date, times), main_keyboard(mosques))
...
def check_prayer_notifications(token, users, notifications):
    ...
    for prayer_name in NOTIFICATION_PRAYERS:
        ...
        text = format_prayer_notification(prayer_name, prayer_time)
